...
bool Node::updatePeeringStrategy(){
	...
	updateFitness();
	nodeRecords[nodeId].fitnessAtPlay=fitness;

	if(numSimulations==1){

		if(iterations>0)
			nodeRecords[nodeId].fitRecord[3].push_back(getActualDF());
	...
	for(it=availablePeeringStrategies.begin();it!=availablePeeringStrategies.end();it++){
		...
		logFile<<"Delta_Fitness of "<<nodeId<<" with strategy "<<*it<<" is "<<deltaFitness<<endl;
#endif

		if(Q!=-1)
			trafficFractions[nodeId][*it][0]=linkValueVector[linkIndex[nodeId][Q].index]/V;
		else
			trafficFractions[nodeId][*it][0]=0;

		trafficFractions[nodeId][*it][1]=0;
		for(itInt=I.begin();itInt!=I.end();itInt++)
			trafficFractions[nodeId][*it][1]+=linkValueVector[linkIndex[nodeId][*itInt].index];

		trafficFractions[nodeId][*it][1]=trafficFractions[nodeId][*it][1]/V;

		trafficFractions[nodeId][*it][2]=0;
		for(itInt=K.begin();itInt!=K.end();itInt++)
			trafficFractions[nodeId][*it][2]+=linkValueVector[linkIndex[nodeId][*itInt].index];

		trafficFractions[nodeId][*it][2]=trafficFractions[nodeId][*it][2]/V;

		fitnessRecord[*it]=fitness;
		deltaFitnessRecord[*it]=deltaFitness;

		nodeRecords[nodeId].fitRecord[*it].push_back(deltaFitnessRecord[*it]);
	...
	for(int i=0;i<7;i++){
		...
		if(isPresentInList(availablePeeringStrategies,i) && fitnessRecord[i]>currentFitness && deltaFitnessRecord[i]>maxDf){
			bestStrategy=i;
			maxDf=deltaFitnessRecord[i];
...
void recordBetweennessCentralityDistribution(){
	...
	ofstream bdFile;
	char bdFileName[50];
	sprintf(bdFileName,"betweennessCentralityDistribution_%d_%d_%d.txt",mySeed,initialTopologySeed,playOrderSeed);
	bdFile.open(bdFileName,ios::out|ios::app);

	ofstream bFile;
	char bFileName[50];
	sprintf(bFileName,"betweennessCentrality_%d_%d_%d.txt",mySeed,initialTopologySeed,playOrderSeed);
...
void recordProviderPeering(){

	ofstream plFile;
	char plFileName[50];
	sprintf(plFileName,"providerLinkLog_%d_%d_%d.txt",mySeed,initialTopologySeed,playOrderSeed);
	plFile.open(plFileName,ios::out|ios::app);

	ofstream ptFile;
	char ptFileName[50];
	sprintf(ptFileName,"providerTrafficLog_%d_%d_%d.txt",mySeed,initialTopologySeed,playOrderSeed);
...
void recordFitnessLog(){

	ofstream fitnessFile;
	char fitnessFileName[50];
	sprintf(fitnessFileName,"fitnessLog_%d_%d_%d.txt",mySeed,initialTopologySeed,playOrderSeed);
...
void recordJaccardLog(){

	ofstream jFile;
	char fileName[50];
	sprintf(fileName,"jaccardLog_%d_%d_%d.txt",mySeed,initialTopologySeed,playOrderSeed);
	jFile.open(fileName);
	int linkType;
...
void recordDistributions(){

	ofstream dFile;
	char fileName[50];
	sprintf(fileName,"distributions_%d_%d_%d.txt",mySeed,initialTopologySeed,playOrderSeed);
	dFile.open(fileName);

	ofstream linkLoadFile;
	char linkfileName[50];
	sprintf(linkfileName,"linkLoadDistribution_%d_%d_%d.txt",mySeed,initialTopologySeed,playOrderSeed);
	linkLoadFile.open(linkfileName);
	list<double> linkLoad;

	ofstream stubDegreeFile;
	char stubDegreefileName[50];
	sprintf(stubDegreefileName,"stubDegree_%d_%d_%d.txt",mySeed,initialTopologySeed,playOrderSeed);
	stubDegreeFile.open(stubDegreefileName);

	ofstream providerDegreeFile;
	char providerDegreefileName[50];
	sprintf(providerDegreefileName,"providerDegree_%d_%d_%d.txt",mySeed,initialTopologySeed,playOrderSeed);
...
void recordEquilibrium(){

	ofstream eFile;
	char fileName[50];
	sprintf(fileName,"equilibrium_%d_%d_%d.txt",mySeed,initialTopologySeed,playOrderSeed);
	eFile.open(fileName);
