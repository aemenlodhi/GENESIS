	...
	while(iterations<maxIterations){
		...
		while((steps=getNewIt())!=-1){
			...
			if(!aliveNodes[steps].K.empty())
				depeeringChange=aliveNodes[steps].dePeer();
		...
		if(stepChange==true){
			...
			if(numSimulations==0){	//first equilibrium

				cout<<"System reached equilibrium 0 after "<<iterations<<" iterations"<<endl;
				logFile<<"System reached equilibrium 0 after "<<iterations<<" iterations"<<endl;
				...
				if(iterations>25){	//simulation with oscillations
					//analyzeOscillations();
				}
					...
					recordJaccardLog();
					recordFitnessLog();
					recordProviderPeering();
					recordBetweennessCentralityDistribution();
				}
